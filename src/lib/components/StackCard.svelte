<script lang="ts">
  ...
  const hervorgehoben = $derived(ui.highlightedIds.has(stack.id));
  ...
  function onPointerDown(e: PointerEvent) {
    ...
    // Ein Stapel ist laut UI-Spezifikation ein zulässiges Quellobjekt.
    if (ui.zeitleisteEintragFuer) {
      starteZeitleisteEintrag(ui.zeitleisteEintragFuer, stack.id);
      return;
    }
    if (dragGesperrt) {
  ...
  function onPointerUp(e: PointerEvent) {
    ...
    dragging = false;
    if (moved) {
      const ziel = abwurfZielFuer(e.clientX, e.clientY, stack.id);
      if (ziel.art === 'korb') {
      ...
      if (ziel.art === 'zeitleiste') {
        ...
        starteZeitleisteEintrag(ziel.zeitleisteId, stack.id);
        activePointer = null;
        return;
      }
  ...
  function fanPointerDown(e: PointerEvent, docId: string) {
    ...
    const onUp = (up: PointerEvent) => {
      ...
      if (Math.hypot(up.clientX - startX, up.clientY - startY) > 30) {
        ...
        // Querformat-Karten liegend zentrieren (Maße getauscht wie in docBox)
        const quer = findDoc(desktop.state, docId)?.landscape === true;
        void desktop.command('removeFromStack', {
          docId,
          position: { x: w.x - (quer ? CARD_H : CARD_W) / 2, y: w.y - (quer ? CARD_W : CARD_H) / 2 },
        });
      } else {
...
{:else}
  <div class="stack" class:hervorgehoben={hervorgehoben} role="button" tabindex="-1" aria-label={stack.name || 'Stapel'}
...
<style>
  ...
  /* Hervorhebungs-Ring (VIEW-01, 11-08): strukturell dem Bearbeitungs-Ring aus DocCard/NoteCard
     ...
     Kontur 2px INNERHALB der Stapelkontur über ::after umgesetzt: verschiebt kein Layout und
     bleibt über den undurchsichtigen Blättern sichtbar (ein inset-Schatten direkt auf .stack
     läge HINTER den .sheet-Kindelementen). */
  .stack.hervorgehoben::after { content: ''; position: absolute; inset: 0; border-radius: 4px;
                                box-shadow: inset 0 0 0 2px var(--brand-blue); pointer-events: none; }
  /* 📌-Badge: oben links AUSSERHALB der Stapelkontur aufgesteckt (Muster .klammer). Bewusst KEIN
     pointer-events: none — der Pflicht-Tooltip braucht den Zeigerkontakt; der Zeigerdruck
     bubbelt zum Stapel und startet dort regulär den Drag. */
