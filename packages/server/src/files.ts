...
/** Auch von jdesk.ts genutzt, um eingebettete Paketdateien schon beim Lesen zu prüfen —
    dieselbe Grenze wie hier, nicht dupliziert. */
...
export function storeFile(db: Db, dataDir: string, bytes: Buffer, originalName: string): FileMeta {
  ...
  if (existing) {
    // Heilt Bestandsdateien, die noch keine Extraktion haben (idempotent, kostet dank der
    // enqueueExtraction-Sperre nur einen SELECT, falls schon extrahiert). Zweite Aufrufstelle:
    // app.ts cachedDocBytes() (j-lawyer-Pfad) — beide Wege müssen die Extraktion auslösen können,
    // weil Dokumente auf zwei unabhängigen Ingestionswegen ins System gelangen (07-RESEARCH
    // Pitfall 4): ein Trigger nur hier würde j-lawyer-synchronisierte Scans nie durchlaufen.
    enqueueExtraction(db, existing.id, existing.kind, async () => bytes);
    return existing;
  }
  ...
  // Standalone-Ingestionsweg (Gegenstück: app.ts cachedDocBytes() im j-lawyer-Pfad, s. Kommentar
  // oben) — fire-and-forget, blockiert die Upload-Antwort nicht (kein await hier).
  enqueueExtraction(db, id, kind, async () => bytes);
  return { id, sha256, originalName, size: bytes.length, kind };
}

/**
 * Sprung zur Quelle im j-lawyer-Modus: berechnet (falls noch nicht geschehen) den SHA-256 der
 * Bytes einer Dokumentfassung und persistiert ihn in jl_file_hashes — idempotent, ein zweiter
 * Aufruf mit denselben Bytes/Schlüssel liefert denselben Hash ohne Insert-Fehler. Analog zu
 * storeFile() (Standalone), aber ohne Datei-Ablage: die Bytes selbst liegen im jl-Cache
 * (app.ts:cachedDocBytes), hier wird nur der Fingerabdruck gepflegt.
 */
export function getOrComputeJlHash(db: Db, docId: string, changeDate: number, bytes: Buffer): string {
  const sha256 = createHash('sha256').update(bytes).digest('hex');
  db.prepare('INSERT OR IGNORE INTO jl_file_hashes (doc_id, change_date, sha256) VALUES (?, ?, ?)').run(docId, changeDate, sha256);
  return sha256;
}
