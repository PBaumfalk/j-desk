...
import { emptyState, isValidState, type Doc } from './model';
...
describe('Doc-Referenzstatus-Felder (jl-Referenzstatus)', () => {
  it('akzeptiert einen Zustand MIT sourceGone/sourceChangeDate/sourceReplacedAt', () => {
    // Typ-Existenz: Doc-Objektbau mit den neuen Feldern muss typchecken (tsc/svelte-check).
    const doc: Doc = {
      id: 'a', fileId: 'f', name: 'a.pdf', position: { x: 0, y: 0 }, rotation: 0, zIndex: 1,
      sourceGone: true,
      sourceChangeDate: 1732000000000,
      sourceReplacedAt: '2026-07-19T12:00:00.000Z',
    };
    const s = { docs: [doc], links: [], stacks: [] };
    expect(isValidState(s)).toBe(true);
  });
});
