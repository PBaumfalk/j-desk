...
describe('Boxen', () => {
  ...
  it('docBox tauscht die Maße bei Querformat-Karten', () => {
    const doc = { id: 'd', fileId: 'f', name: 'n', position: { x: 5, y: 6 }, rotation: 0, zIndex: 1, landscape: true as const };
    expect(docBox(doc)).toEqual({ x: 5, y: 6, w: CARD_H, h: CARD_W });
  });

  it('docBox: aufgeschlagen gewinnt openSize auch bei landscape', () => {
    const doc = { id: 'd', fileId: 'f', name: 'n', position: { x: 0, y: 0 }, rotation: 0, zIndex: 1, landscape: true as const, open: true, openSize: { w: 500, h: 400 } };
    expect(docBox(doc)).toEqual({ x: 0, y: 0, w: 500, h: 400 });
  });
});
