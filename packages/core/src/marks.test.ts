...
describe('marks', () => {
  ...
  it('übernimmt fileSha256, wenn gesetzt, und lässt es weg, wenn leer/fehlend', () => {
    const mitHash = addMark(mitDoc(), { docId: 'd1', page: 1, rect: { x: 0, y: 0, w: 5, h: 5 }, kind: 'tippex', fileSha256: 'abc123' });
    expect(mitHash.marks?.[0].fileSha256).toBe('abc123');
    const leererHash = addMark(mitDoc(), { docId: 'd1', page: 1, rect: { x: 0, y: 0, w: 5, h: 5 }, kind: 'tippex', fileSha256: '' });
    expect(leererHash.marks?.[0]).not.toHaveProperty('fileSha256');
    const fehlenderHash = addMark(mitDoc(), { docId: 'd1', page: 1, rect: { x: 0, y: 0, w: 5, h: 5 }, kind: 'tippex' });
    expect(fehlenderHash.marks?.[0]).not.toHaveProperty('fileSha256');
  });
