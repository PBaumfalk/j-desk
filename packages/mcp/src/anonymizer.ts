...
export class Anonymizer {
  ...
  /** Anonymisiert einen BEREITS extrahierten Text (12-05: Seitentext der file-text-Route,
   *  der Server hat pdf/ocr bereits aufgelöst) — dasselbe Mapping wie anonFileText, aber
   *  ohne Datei-Upload. Kein Cache: Seitentexte sind klein, der Agent ruft seitenweise ab. */
  async anonText(text: string): Promise<string> {
    const r = await this.client.anonymizeText(text);
    this.mappings.record(r.pairs);
    return r.text.length > MAX_TEXT ? `${r.text.slice(0, MAX_TEXT)}\n\n[Hinweis: Text gekürzt]` : r.text;
  }
}
